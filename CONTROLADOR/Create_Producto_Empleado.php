...
if (mysqli_num_rows($resultCheck) > 0) {
    ...
    if ($execute) {
        ...
        header("Location: ../VISTA/Vista_Create_ProductoEmpleado.php?success=" . urlencode($success));
        exit();
    } else {
        // Redirigir con mensaje de error
        $error = "Error al registrar el producto: " . mysqli_error($conn);
        header("Location: ../VISTA/Vista_Create_ProductoEmpleado.php?error=" . urlencode($error));
        exit();
    }
}
?>
