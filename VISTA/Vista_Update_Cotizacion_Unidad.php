...
<section class="texto-filtros-container">
    ...
    <section class="filtros">
        ...
        <form action="" method="GET">
            ...
            <button type="button" onclick="location.href='Vista_Update_Cotizacion_Unidad.php'">Volver</button>
        </form>
    </section>
</section>
...
<section class="reportes">
    ...
    <section class="botones-reportes">
        ...
        <div class="modal" id="modalTotalCotizacionesPorUnidadCliente" style="display: none;">
            <div class="modal-content">
                ...
                <br>
                <button type="button" onclick="cerrarModal('modalTotalCotizacionesPorUnidadCliente')">Cancelar</button>
            </div>
        </div>
        ...
        <button onclick="mostrarModal3()">Cliente que más productos cotizó</button>
        <div class="modal" id="modalClienteMasProductosCotizoUnidad" style="display: none;">
            <div class="modal-content">
                <h2>Cliente que más productos cotizó</h2>
                <p>Selecciona el mes y el año en el que se trabajará su reporte.</p>
...
<script>
    function mostrarModal() {
        document.getElementById('modalTotalCotizacionesPorUnidadCliente').style.display = 'flex';
    }
...
function confirmarEliminacion(idCotizacion) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch("../CONTROLADOR/Delete_Cotizacion_Unidad.php?IDCotizacion=" + idCotizacion)
                .then(response => {
                    if (response.ok) {
                        Swal.fire(
                            'Eliminado',
...
function confirmarRevisada(idCotizacion) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch("../CONTROLADOR/Update_Cotizacion_Unidad_Revisada.php?IDCotizacion=" + idCotizacion)
                .then(response => {
                    if (response.ok) {
                        Swal.fire(
                            'Revisada',
...
function confirmarRechazo(idCotizacion) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch("../CONTROLADOR/Update_Cotizacion_Unidad_Rechazada.php?IDCotizacion=" + idCotizacion)
                .then(response => {
                    if (response.ok) {
                        Swal.fire(
                            'Rechazada',
