...
<section class="texto-filtros-container">
    ...
    <section class="filtros">
        ...
        <form action="" method="GET">
            ...
            <button type="button" onclick="location.href='Vista_Update_Cotizacion_Lote_Empleado.php'">Volver</button>
        </form>
    </section>
</section>
...
<hr>
<h3>Reportes</h3>
<a href="../VISTA/EXCEL/Reporte_Total_Cotizaciones_Lote_Por_Cliente.php">Total de cotizaciones por lote de clientes</a>
<a href="../VISTA/EXCEL/Reporte_Producto_Mas_Cotizado_Lote.php">Producto más cotizado por lote</a>
<a href="../VISTA/EXCEL/Reporte_Cliente_Mas_Productos_Cotizo_Lote.php">Cliente que más productos cotizó por lote en el último mes</a>
<hr>
</section>
...
function confirmarEliminacion(idCotizacion) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch("../CONTROLADOR/Delete_Cotizacion_Lote_Empleado.php?IDCotizacion=" + idCotizacion)
                .then(response => {
                    if (response.ok) {
                        Swal.fire(
                            'Eliminado',
...
function confirmarRevisada(idCotizacion) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch("../CONTROLADOR/Update_Cotizacion_Lote_Revisada_Empleado.php?IDCotizacion=" + idCotizacion)
                .then(response => {
                    if (response.ok) {
                        Swal.fire(
                            'Revisada',
...
function confirmarRechazo(idCotizacion) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch("../CONTROLADOR/Update_Cotizacion_Lote_Rechazada_Empleado.php?IDCotizacion=" + idCotizacion)
                .then(response => {
                    if (response.ok) {
                        Swal.fire(
                            'Rechazada',
